"""
Bitget COIN-M срочные (delivery): productType COIN-FUTURES, symbolType delivery.
Публичные книги через тот же v2 ws с instType COIN-FUTURES.
Исполнение — отдельный маршрут пока не подключён; для спреда доступны L1-котировки.
"""
...
class BitgetCoinDeliveryInstrumentLoader:
    EXCHANGE = "bitget"
    ...
    def load_instruments(self) -> list[InstrumentId]:
        payload = self._client.get(self.INSTRUMENTS_PATH, params={"productType": "COIN-FUTURES"})
    ...
    def _build_instrument(self, item: dict[str, Any]) -> InstrumentId | None:
        ...
        if not symbol or symbol_type != "delivery" or symbol_status != "normal":
            return None
        ...
        key = InstrumentKey(exchange=self.EXCHANGE, market_type="bitget_coin_delivery", symbol=symbol)
        spec = InstrumentSpec(
            base_asset=base_asset,
            quote_asset=quote_asset or "USD",
            contract_type="delivery",
            settle_asset=base_asset,
