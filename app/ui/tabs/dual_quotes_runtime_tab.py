...
class DualQuotesRuntimeTab(QWidget):
    ...
    def _on_instruments_loaded(self, exchange: str, market_type: str) -> None:
        ...
        if exchange == "binance":
            ...
            self._left_symbols = list(self._left_display_to_symbol.keys())
            self.left_model.setStringList(self._left_symbols[:12])
            ...
            self._right_symbols = list(self._right_display_to_symbol.keys())
            self.right_model.setStringList(self._right_symbols[:12])
    ...
    def _filter_symbols(text: str, symbols: list[str], model: QStringListModel, completer: QCompleter) -> None:
        ...
        if query:
            starts = [item for item in symbols if item.startswith(query)]
            contains = [item for item in symbols if query in item and not item.startswith(query)]
            filtered = [*starts, *contains][:12]
        else:
            filtered = symbols[:12]
    ...
    def _on_left_symbol_selected(self, symbol: str) -> None:
        self.left_symbol_input.setText(str(symbol or "").upper())
        self.left_completer.popup().hide()

    def _on_right_symbol_selected(self, symbol: str) -> None:
        self.right_symbol_input.setText(str(symbol or "").upper())
    ...
    def _start_runtime(self) -> None:
        ...
        if left_credentials is None or right_credentials is None:
            self._append_log(tr("dual.error.no_credentials"))
            return
        left_symbol = self.coordinator.resolve_instrument_symbol("binance", "perpetual", self.left_symbol_input.text())
        right_symbol = self.coordinator.resolve_instrument_symbol("bitget", "perpetual", self.right_symbol_input.text())
        if not left_symbol or not right_symbol:
            self._append_log(tr("dual.error.select_both"))
            return
        self._append_log(tr("dual.log.starting", left_symbol=left_symbol, right_symbol=right_symbol))
        self.action_triggered.emit("dual_runtime:start")
        self.coordinator.start_dual_execution_runtime_async(
            worker_id=self.worker_id,
            left_exchange="binance",
            left_market_type="perpetual",
            left_symbol=left_symbol,
            left_api_key=left_credentials["api_key"],
            left_api_secret=left_credentials["api_secret"],
            left_api_passphrase=left_credentials.get("api_passphrase", ""),
            ...
            right_api_key=right_credentials["api_key"],
            right_api_secret=right_credentials["api_secret"],
            right_api_passphrase=right_credentials.get("api_passphrase", ""),
    ...
    def _start_spread_entry_runtime(self) -> None:
        ...
        self._append_log(
            tr(
                "dual.log.starting_spread_entry",
                left_symbol=left_symbol,
                right_symbol=right_symbol,
                threshold=self.entry_threshold_input.text().strip(),
            )
        )
        ...
        self.coordinator.start_spread_entry_runtime_async(
            ...
            right_api_passphrase=right_credentials.get("api_passphrase", ""),
            right_account_profile=self._runtime_account_profile("bitget", right_credentials),
            entry_threshold=self.entry_threshold_input.text().strip(),
            max_quote_age_ms=self.max_quote_age_input.text().strip(),
            max_quote_skew_ms=self.max_quote_skew_input.text().strip(),
    ...
    def _stop_runtime(self) -> None:
        ...
        self._append_log(tr("dual.log.stopping"))
