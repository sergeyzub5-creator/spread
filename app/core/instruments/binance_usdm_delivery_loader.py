"""
Загрузка срочных (доставочных) USD-M контрактов Binance: CURRENT_QUARTER / NEXT_QUARTER.
Тот же fapi/bookTicker и trade WS, что у бессрочных — другой contractType в exchangeInfo.
"""
...
# Типы контрактов Binance USD-M, отличные от PERPETUAL (срочные квартальные и т.д.)
_DELIVERY_CONTRACT_TYPES = frozenset({"CURRENT_QUARTER", "NEXT_QUARTER"})


class BinanceUsdmDeliveryInstrumentLoader:
    EXCHANGE = "binance"
    ...
    def _build_instrument(self, symbol_info: dict[str, Any]) -> InstrumentId | None:
        ...
        if not symbol or status != "TRADING" or contract_type not in _DELIVERY_CONTRACT_TYPES:
            return None
        # Символы вида BTCUSDT_260327 — маржа в USDT
        if str(symbol_info.get("quoteAsset", "")).upper() != "USDT":
            return None
