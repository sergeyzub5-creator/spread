...
class BybitLinearPublicConnector(PublicMarketDataConnector):
        ...
        with self._lock:
            if self._thread is not None and self._thread.is_alive():
                ...
                if self._connected or self._last_disconnect_ts_ms <= 0 or disconnected_for_ms < 2000:
                    return
                self.logger.warning(
                    "bybit public ws hard reconnect | disconnected_for_ms=%s",
                    disconnected_for_ms,
                )
                self._closing = True
    ...
    def _on_message(self, message: str) -> None:
        ...
        for callback in list(self._callbacks):
            try:
                ...
                self.logger.error("bybit linear quote callback failed: %s", exc)
    ...
    def _on_error(self, error: Any) -> None:
        ...
        if self._closing:
            self.logger.info("bybit public ws closing | error=%s", error_text)
            return
        if error_text in {"Connection to remote host was lost.", "socket is already closed."}:
            self.logger.warning("bybit public ws disconnected: %s", error_text)
            return
    ...
    def _start_watchdog_loop(self) -> None:
        with self._lock:
            ...
            self._watchdog_thread = threading.Thread(
                ...
                name="bybit-linear-public-watchdog",
                daemon=True,
            )
    ...
    def _watchdog_loop(self) -> None:
        ...
        while not self._closing and self._connected:
            ...
            if last_message_ts_ms > 0 and (now_ms - last_message_ts_ms) >= self.STALE_STREAM_TIMEOUT_MS:
                self.logger.warning(
                    "bybit linear public ws stale stream detected | silence_ms=%s | action=restart",
    ...
    def _send(self, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            self._connected = False
            self._last_disconnect_ts_ms = int(time.time() * 1000)
            self.logger.warning("bybit public ws send failed: %s", exc)
            try:
                ws_app.close()
            except Exception:
                pass
