...
def load_telegram_credentials(credential_ref: str | None) -> dict[str, str] | None:
    ref = str(credential_ref or "").strip()
    if not ref:
        return None
    with _LOCK:
    ...
    if payload is None:
        return None
    if str(payload.get("kind", "")).strip().lower() not in {"", "telegram"}:
        return None
...
def delete_telegram_credentials(credential_ref: str | None) -> None:
    delete_exchange_credentials(credential_ref)


def has_telegram_credentials(credential_ref: str | None) -> bool:
    return load_telegram_credentials(credential_ref) is not None


def find_telegram_credential_ref() -> str | None:
    with _LOCK:
        store = _load_store()
        credentials = dict(store.get("credentials") or {})
    for ref in reversed(list(credentials.keys())):
        record = credentials.get(ref)
        if not isinstance(record, dict):
            continue
        kind = str(record.get("kind", "")).strip().lower()
        if kind not in {"", "telegram"}:
            continue
        if load_telegram_credentials(ref) is not None:
