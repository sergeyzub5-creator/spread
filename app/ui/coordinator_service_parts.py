...
class RestAccountMonitor:
    _STOP_JOIN_TIMEOUT_SECONDS = 2.0

    def __init__(self, credentials: ExchangeCredentials, connector, *, poll_interval_seconds: float = 1.0) -> None:
    ...
    def start(self, on_snapshot, on_error) -> None:
        with self._lock:
            ...
            self._thread = threading.Thread(target=self._run_loop, args=(on_snapshot, on_error), name=f"{self._credentials.exchange}-account-monitor-poll", daemon=True)
            self._thread.start()
    ...
    def _run_loop(self, on_snapshot, on_error) -> None:
        self._logger.info("%s account rest monitor loop started | interval_seconds=%s", self._credentials.exchange, self._poll_interval_seconds)
        try:
...
class UiCoordinatorPartsMixin:
    ...
    def available_execution_routes(self, exchange: str, account_profile: dict | None = None) -> list[tuple[str, str]]:
    ...
    def list_instrument_items(self, exchange: str, market_type: str) -> list[dict[str, str]]:
        ...
        return [{"symbol": instrument.symbol, "display": self._display_symbol(instrument), "quote_asset": str(instrument.spec.quote_asset or instrument.spec.settle_asset or "").strip().upper()} for instrument in instruments]
    ...
    def connect_exchange_async(self, request_id: str, exchange: str, params: dict) -> None:
        ...
        self._submit_background(f"exchange-connect-{exchange_code}", _run)
    ...
    def close_exchange_positions_async(self, request_id: str, exchange: str, params: dict) -> None:
        ...
        self._submit_background(f"exchange-close-positions-{exchange_code}", _run)
    ...
    def start_exchange_monitor(self, monitor_id: str, exchange: str, params: dict) -> None:
        ...
        if exchange_code not in {"binance", "bitget", "bybit"}:
            return
        credentials = ExchangeCredentials(exchange=exchange_code, api_key=str(params.get("api_key", "")), api_secret=str(params.get("api_secret", "")), api_passphrase=str(params.get("api_passphrase", "")))
        monitor = RestAccountMonitor(credentials, self._account_connector_for_exchange(exchange_code))
        ...
        def _run() -> None:
            try:
                ...
                try:
                    ...
                    self.stop_exchange_monitor(monitor_id)
        self._submit_background(f"exchange-monitor-{exchange_code}", _run)
    ...
    def stop_all_exchange_monitors(self) -> None:
        with self._monitor_lock:
            monitor_ids = list(self._account_monitors.keys())
    ...
    def shutdown(self) -> None:
        if self._shutdown:
            return
        self._shutdown = True
        self._logger.info("ui coordinator shutdown started")
        with self._subscription_lock:
            slot_names = list(self._subscriptions.keys())
        for slot_name in slot_names:
            self.unsubscribe_public_quote(slot_name)
        self.stop_all_exchange_monitors()
        self.event_bus.unsubscribe("worker_state", self._on_worker_state)
        self.event_bus.unsubscribe("worker_events", self._on_worker_event)
        self.worker_manager.shutdown()
        self.market_data_service.shutdown()
        with self._async_lock:
            pending_futures = list(self._pending_futures)
        if pending_futures:
            wait(pending_futures, timeout=2.0)
        # Do not block app shutdown forever on stuck background tasks.
        self._async_executor.shutdown(wait=False, cancel_futures=True)
        self._logger.info("ui coordinator shutdown completed")

    def start_test_runtime_async(self, *, worker_id: str, exchange: str, market_type: str, symbol: str, api_key: str, api_secret: str, api_passphrase: str = "", account_profile: dict | None = None, target_notional: str = "10") -> None:
        self._logger.info("ui start test runtime requested | worker_id=%s | exchange=%s | market_type=%s | symbol=%s | target_notional=%s", worker_id, exchange, market_type, symbol, target_notional)
        def _run() -> None:
            try:
                instrument = self._resolve_instrument(exchange, market_type, symbol)
                normalized_target_notional = parse_decimal_text(target_notional, default="10")
                task = WorkerTask(worker_id=worker_id, left_instrument=instrument, right_instrument=instrument, entry_threshold=Decimal("0"), exit_threshold=Decimal("0"), target_notional=normalized_target_notional, step_notional=normalized_target_notional, execution_mode="test_manual", run_mode="single_instrument_test", execution_credentials=ExchangeCredentials(exchange=str(exchange or "").strip().lower(), api_key=str(api_key or "").strip(), api_secret=str(api_secret or "").strip(), api_passphrase=str(api_passphrase or "").strip(), account_profile=dict(account_profile or {})))
        ...
        self._submit_background(f"worker-start-{worker_id}", _run)

    def stop_test_runtime(self, worker_id: str) -> None:
        self._logger.info("ui stop runtime requested | worker_id=%s", worker_id)
        def _run() -> None:
            try:
                self.worker_manager.stop_worker(worker_id)
                self._logger.info("ui stop runtime completed | worker_id=%s", worker_id)
            except Exception as exc:
                self._logger.error("ui stop runtime failed | worker_id=%s | error=%s", worker_id, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"worker-stop-{worker_id}", _run)

    def execution_stream_health_snapshot(self, worker_id: str) -> dict:
        return self.worker_manager.execution_stream_health_snapshot(worker_id)

    def start_dual_quotes_runtime_async(self, *, worker_id: str, left_exchange: str, left_market_type: str, left_symbol: str, right_exchange: str, right_market_type: str, right_symbol: str) -> None:
        self._logger.info("ui start dual quotes runtime requested | worker_id=%s | left=%s:%s:%s | right=%s:%s:%s", worker_id, left_exchange, left_market_type, left_symbol, right_exchange, right_market_type, right_symbol)
        def _run() -> None:
            try:
                ...
                task = WorkerTask(worker_id=worker_id, left_instrument=left_instrument, right_instrument=right_instrument, entry_threshold=Decimal("0"), exit_threshold=Decimal("0"), target_notional=Decimal("0"), step_notional=Decimal("0"), execution_mode="quotes_only", run_mode="dual_exchange_quotes", execution_credentials=None)
                self.worker_manager.start_worker(task)
            except Exception as exc:
                self._logger.error("dual quotes worker start failed | worker_id=%s | error=%s", worker_id, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"dual-worker-start-{worker_id}", _run)

    def start_dual_execution_runtime_async(self, *, worker_id: str, left_exchange: str, left_market_type: str, left_symbol: str, left_api_key: str, left_api_secret: str, left_api_passphrase: str = "", left_account_profile: dict | None = None, right_exchange: str, right_market_type: str, right_symbol: str, right_api_key: str, right_api_secret: str, right_api_passphrase: str = "", right_account_profile: dict | None = None) -> None:
        self._logger.info("ui start dual execution runtime requested | worker_id=%s | left=%s:%s:%s | right=%s:%s:%s", worker_id, left_exchange, left_market_type, left_symbol, right_exchange, right_market_type, right_symbol)
        def _run() -> None:
            try:
                left_instrument = self._resolve_instrument(left_exchange, left_market_type, left_symbol)
                right_instrument = self._resolve_instrument(right_exchange, right_market_type, right_symbol)
                task = WorkerTask(worker_id=worker_id, left_instrument=left_instrument, right_instrument=right_instrument, entry_threshold=Decimal("0"), exit_threshold=Decimal("0"), target_notional=Decimal("0"), step_notional=Decimal("0"), execution_mode="manual_dual_test", run_mode="dual_exchange_test_execution", execution_credentials=None, left_execution_credentials=ExchangeCredentials(exchange=str(left_exchange or "").strip().lower(), api_key=str(left_api_key or "").strip(), api_secret=str(left_api_secret or "").strip(), api_passphrase=str(left_api_passphrase or "").strip(), account_profile=dict(left_account_profile or {})), right_execution_credentials=ExchangeCredentials(exchange=str(right_exchange or "").strip().lower(), api_key=str(right_api_key or "").strip(), api_secret=str(right_api_secret or "").strip(), api_passphrase=str(right_api_passphrase or "").strip(), account_profile=dict(right_account_profile or {})))
                self.worker_manager.start_worker(task)
            except Exception as exc:
    ...
    def start_spread_entry_runtime_async(self, *, worker_id: str, left_exchange: str, left_market_type: str, left_symbol: str, left_api_key: str, left_api_secret: str, left_api_passphrase: str = "", left_account_profile: dict | None = None, right_exchange: str, right_market_type: str, right_symbol: str, right_api_key: str, right_api_secret: str, right_api_passphrase: str = "", right_account_profile: dict | None = None, entry_threshold: str, exit_threshold: str, max_quote_age_ms: str, max_quote_skew_ms: str, max_slippage_pct: str = "0", entry_notional_usdt: str = "0", step_notional_usdt: str = "0", entry_min_step_pct: str = "20", left_qty: str = "0", right_qty: str = "0", strategy_signal_mode: str = "market", mid_alarm_enabled: str = "1", mid_alarm_window_sec: str = "60") -> None:
        ...
        def _run() -> None:
            try:
                left_instrument = self._resolve_instrument(left_exchange, left_market_type, left_symbol)
                right_instrument = self._resolve_instrument(right_exchange, right_market_type, right_symbol)
                normalized_entry_threshold = normalize_decimal_text(entry_threshold)
                normalized_exit_threshold = normalize_decimal_text(exit_threshold)
                normalized_max_slippage_pct = normalize_decimal_text(max_slippage_pct)
                normalized_entry_notional = normalize_decimal_text(entry_notional_usdt)
                normalized_cycle_count_text = normalize_decimal_text(step_notional_usdt)
                normalized_entry_min_step_pct_raw = parse_decimal_text(normalize_decimal_text(entry_min_step_pct), default="20")
                normalized_entry_min_step_pct = max(Decimal("10"), min(Decimal("100"), normalized_entry_min_step_pct_raw))
                ...
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"spread-entry-worker-start-{worker_id}", _run)

    def submit_test_order_async(self, worker_id: str, side: str, submitted_at_ms: int | None = None) -> None:
        self._logger.info("ui submit test order requested | worker_id=%s | side=%s | submitted_at_ms=%s", worker_id, side, submitted_at_ms)
        def _run() -> None:
            try:
                ...
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"worker-order-{worker_id}-{side}", _run)

    def submit_dual_test_orders_async(self, *, worker_id: str, left_side: str, right_side: str, left_qty: str, right_qty: str, left_price_mode: str = "top_of_book", right_price_mode: str = "top_of_book", submitted_at_ms: int | None = None) -> None:
        self._logger.info("ui submit dual test orders requested | worker_id=%s | left=%s:%s | right=%s:%s", worker_id, left_side, left_qty, right_side, right_qty)
        def _run() -> None:
            try:
                self.worker_manager.submit_dual_test_orders(worker_id, left_side=left_side, right_side=right_side, left_qty=left_qty, right_qty=right_qty, left_price_mode=left_price_mode, right_price_mode=right_price_mode, submitted_at_ms=submitted_at_ms)
            except Exception as exc:
                self._logger.error("dual worker order failed | worker_id=%s | error=%s", worker_id, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"dual-worker-order-{worker_id}", _run)

    def trigger_entry_signal_async(self, worker_id: str) -> None:
        self._logger.info("ui manual entry trigger requested | worker_id=%s", worker_id)
        def _run() -> None:
            try:
                self.worker_manager.trigger_entry_signal(worker_id)
            except Exception as exc:
                self._logger.error("manual entry trigger failed | worker_id=%s | error=%s", worker_id, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"worker-trigger-entry-{worker_id}", _run)

    def set_simulated_entry_window_async(self, worker_id: str, enabled: bool) -> None:
        self._logger.info("ui simulated entry window requested | worker_id=%s | enabled=%s", worker_id, enabled)
        def _run() -> None:
            try:
                self.worker_manager.set_simulated_entry_window(worker_id, enabled)
            except Exception as exc:
                self._logger.error("simulated entry window failed | worker_id=%s | enabled=%s | error=%s", worker_id, enabled, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"worker-sim-entry-window-{worker_id}", _run)

    def set_simulated_exit_window_async(self, worker_id: str, enabled: bool) -> None:
        self._logger.info("ui simulated exit window requested | worker_id=%s | enabled=%s", worker_id, enabled)
        def _run() -> None:
            try:
                self.worker_manager.set_simulated_exit_window(worker_id, enabled)
            except Exception as exc:
                self._logger.error("simulated exit window failed | worker_id=%s | enabled=%s | error=%s", worker_id, enabled, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"worker-sim-exit-window-{worker_id}", _run)

    def set_strategy_signal_mode_async(self, worker_id: str, mode: str) -> None:
        self._logger.info("ui strategy signal mode requested | worker_id=%s | mode=%s", worker_id, mode)
        def _run() -> None:
            try:
                self.worker_manager.set_strategy_signal_mode(worker_id, mode)
            except Exception as exc:
                self._logger.error("strategy signal mode failed | worker_id=%s | mode=%s | error=%s", worker_id, mode, exc)
                self.worker_command_failed.emit(worker_id, str(exc))
        self._submit_background(f"worker-signal-mode-{worker_id}", _run)
    ...
    def _connect_exchange(self, exchange: str, params: dict):
        credentials = ExchangeCredentials(exchange=exchange, api_key=str(params.get("api_key", "")), api_secret=str(params.get("api_secret", "")), api_passphrase=str(params.get("api_passphrase", "")))
        if exchange == "binance":
    ...
    def _close_exchange_positions(self, exchange: str, params: dict):
        self.worker_manager.mark_rebalance_grace_for_exchange(exchange)
        credentials = ExchangeCredentials(exchange=exchange, api_key=str(params.get("api_key", "")), api_secret=str(params.get("api_secret", "")), api_passphrase=str(params.get("api_passphrase", "")))
        if exchange == "binance":
    ...
    def _prefetch_market_type(self, exchange: str, market_type: str) -> None:
        ...
        self._submit_background(f"prefetch-{normalized_key[0]}-{normalized_key[1]}", _run_prefetch)
    ...
    def _resolve_instrument(self, exchange: str, market_type: str, symbol: str):
        ...
        instrument = self.instrument_registry.find_by_ui_symbol(normalized_exchange, symbol=normalized_symbol, ui_market_type=normalized_market_type)
        if instrument is None:
    ...
    def _on_worker_event(self, event: object) -> None:
        if isinstance(event, WorkerEvent):
            ...
            if event.event_type in {"execution_stream_health_updated", "execution_stream_health_warning"}:
                self.execution_stream_health_updated.emit(event.worker_id, dict(event.payload))
