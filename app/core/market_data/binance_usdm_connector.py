...
class BinanceUsdmPublicConnector(PublicMarketDataConnector):
        ...
        with self._lock:
            if self._thread is not None and self._thread.is_alive():
                ...
                if self._connected or self._last_disconnect_ts_ms <= 0 or disconnected_for_ms < 2000:
                    return
                # Connector can stay in a dead half-open state after transport EOF.
                # Force-restart websocket thread when it remains disconnected.
                self.logger.warning(
                    "binance public ws hard reconnect | disconnected_for_ms=%s",
                    disconnected_for_ms,
                )
                self._closing = True
    ...
    def _start_watchdog_loop(self) -> None:
        with self._lock:
            if self._watchdog_thread is not None and self._watchdog_thread.is_alive():
                return
            self._watchdog_thread = threading.Thread(target=self._watchdog_loop, name="binance-usdm-public-watchdog", daemon=True)
    ...
    def _watchdog_loop(self) -> None:
        while not self._closing and self._connected:
            ...
            if last_message_ts_ms > 0 and (now_ms - last_message_ts_ms) >= self.STALE_STREAM_TIMEOUT_MS:
                self.logger.warning(
                    "binance public ws stale stream detected | silence_ms=%s | action=restart",
    ...
    def _send(self, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            # Transport can be torn down by remote side between connection checks and send().
            # Convert this into a soft reconnect path instead of bubbling raw websocket traceback.
            self.logger.warning("binance public ws send failed: %s", exc)
            with self._lock:
                self._connected = False
                self._last_disconnect_ts_ms = int(time.time() * 1000)
            try:
