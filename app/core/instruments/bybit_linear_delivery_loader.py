"""
Bybit linear срочные контракты: contractType LinearFutures (напр. BTCUSDT-13MAR26).
Тот же v5 public linear и order_route, что у бессрочных.
"""
...
class BybitLinearDeliveryInstrumentLoader:
    EXCHANGE = "bybit"
    ...
    def _build_instrument(self, item: dict[str, Any]) -> InstrumentId | None:
        ...
        if not symbol or status != "TRADING":
            return None
        ct_upper = contract_type.upper().replace("_", "")
        if ct_upper != "LINEARFUTURES" and "LINEARFUTURES" not in ct_upper:
            return None
        ...
        key = InstrumentKey(exchange=self.EXCHANGE, market_type="linear_delivery", symbol=symbol)
        spec = InstrumentSpec(
