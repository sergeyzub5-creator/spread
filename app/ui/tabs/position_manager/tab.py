...
from PySide6.QtWidgets import QApplication, QCompleter, QFrame, QGridLayout, QHBoxLayout, QLabel, QLineEdit, QMenu, QPushButton, QScrollArea, QSizePolicy, QVBoxLayout, QWidget

from app.ui.exchange_store import load_exchange_cards
from app.ui.i18n import tr
from app.ui.tabs.position_manager.models import PositionManagerUiState
from app.ui.tabs.position_manager.widgets import PairLineEdit
from app.ui.theme import theme_color
from app.ui.widgets.exchange_badge import build_exchange_icon


class PositionManagerTab(QWidget):
    ...
    def __init__(self, coordinator=None, parent=None) -> None:
        ...
        self._ui_state = PositionManagerUiState()
        self._slot_state = self._ui_state.slots
        ...
        self.apply_theme()
        self.retranslate_ui()
        self._apply_fixed_direction_tone()
    ...
    @staticmethod
    def _format_ui_price(value: object) -> str:
        decimal_value = PositionManagerTab._decimal_or_none(value)
    ...
    @staticmethod
    def _safe_edge(sell_price: Decimal | None, buy_price: Decimal | None) -> Decimal | None:
        if sell_price is None or buy_price is None or buy_price <= Decimal("0"):
            return None
    ...
    def _style_completer_popup(self, completer: QCompleter) -> None:
        completer.popup().setStyleSheet(
            ...
            QListView::item {{ padding: 8px 10px; margin: 2px 0; border-radius: 8px; }}
            QListView::item:hover {{
                background: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 {self._rgba(theme_color('accent'), 0.16)}, stop: 1 {self._rgba(theme_color('surface'), 0.18)});
                border: 1px solid {self._rgba(theme_color('accent'), 0.38)};
            }}
            QListView::item:selected {{
                background: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 {self._rgba(theme_color('accent'), 0.22)}, stop: 1 {self._rgba(theme_color('surface'), 0.24)});
                border: 1px solid {self._rgba(theme_color('accent'), 0.54)};
                color: {theme_color('text_primary')};
            }}
            QScrollBar:vertical {{ background: transparent; width: 8px; margin: 6px 4px 6px 0; }}
            QScrollBar::handle:vertical {{ background: {self._rgba(theme_color('accent'), 0.34)}; border-radius: 4px; min-height: 20px; }}
    ...
    def _transport_state_for_exchange(self, exchange: str | None) -> dict[str, dict[str, str | bool]]:
        ...
        if normalized == "bitget":
            ...
            return {
                "ws": {"active": bool(is_uta), "available": bool(is_uta), "reason": "" if is_uta else tr("spread.transport.bitget_ws_unavailable")},
                "rest": {"active": not is_uta, "available": True, "reason": ""},
            }
        if normalized == "bybit":
            return {
                "ws": {"active": True, "available": True, "reason": ""},
    ...
    def _update_transport_widget(self, slot_name: str) -> None:
        ...
        transport_state = self._transport_state_for_exchange(exchange)
        for route_name in ("ws", "rest"):
            button = controls.get(f"{route_name}_button")
            dot = controls.get(f"{route_name}_dot")
            if button is None or dot is None:
                continue
            route_state = transport_state.get(route_name, {})
            available = bool(route_state.get("available"))
            active = bool(route_state.get("active"))
            reason = str(route_state.get("reason") or "")
    ...
    def _build_leg_column(self, slot_name: str) -> QWidget:
        ...
        exchange_text = tr("position_manager.choose_buy_exchange") if slot_name == "left" else tr("position_manager.choose_sell_exchange")
        exchange_btn = self._make_selector_button(exchange_text, "exchangeSelector", slot_name, self._open_exchange_menu)
        ...
        self._style_completer_popup(pair_completer)
        ...
        pair_row.addWidget(self._build_transport_widget(slot_name, "ws"), 0, 0, Qt.AlignmentFlag.AlignCenter)
        pair_row.addWidget(pair_input, 0, 1, Qt.AlignmentFlag.AlignCenter)
        pair_row.addWidget(self._build_transport_widget(slot_name, "rest"), 0, 2, Qt.AlignmentFlag.AlignCenter)
    ...
    def _build_ui(self) -> None:
        ...
        center.setProperty("edgeTone", "left_cheap")
        ...
        self._select_button = QPushButton(); self._select_button.setObjectName("spreadActionButton")
        self._select_button.setCursor(Qt.CursorShape.ArrowCursor)
        self._select_button.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self._select_button.clicked.connect(self._subscribe_both)
        inner_layout.addWidget(self._select_button)

        self._spread_value_label = QLabel("--")
        self._spread_value_label.setObjectName("spreadValueLabel")
        self._spread_value_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        inner_layout.addWidget(self._spread_value_label)
        ...
        self._strategy_title_label = QLabel(); self._strategy_title_label.setObjectName("strategyTitle")
        strategy_layout.addWidget(self._strategy_title_label)

        grid = QGridLayout()
        grid.setHorizontalSpacing(6)
        for idx, (label_key, value) in enumerate((("spread.entry_threshold", "0.20"), ("spread.exit_threshold", "0.08"), ("spread.target_size", "10"), ("spread.entry_min_step_pct", "20"))):
            cell = QFrame(); cell.setObjectName("strategyFieldCapsule")
            cell_layout = QHBoxLayout(cell)
            cell_layout.setContentsMargins(10, 4, 10, 4)
            label = QLabel(); label.setObjectName("strategyFieldInlineLabel")
            label.setProperty("i18n_key", label_key)
            self._strategy_field_labels.append(label)
            divider = QFrame(); divider.setObjectName("strategyFieldDivider")
            divider.setFixedWidth(1)
            edit = QLineEdit(value); edit.setObjectName("strategyFieldInput")
        ...
        runtimes_scroll = QScrollArea(); runtimes_scroll.setObjectName("runtimesScroll")
        ...
        runtimes_layout = QVBoxLayout(runtimes_container)
        runtimes_layout.setContentsMargins(0, 0, 0, 0)
        runtimes_layout.setSpacing(8)
        runtimes_layout.setAlignment(Qt.AlignmentFlag.AlignTop)
        runtimes_layout.addStretch(1)
    ...
    def _on_pair_field_clicked(self, slot_name: str) -> None:
        self._update_pair_clear_action(slot_name)
        if not self._pair_inputs[slot_name].text().strip():
            self._show_top_instruments(slot_name)
    ...
    def _update_pair_suggestions(self, slot_name: str, text: str, force_popup: bool = False) -> None:
        ...
        if query:
            starts_with = [display for display in displays if display.startswith(query) or display_map.get(display, "") == query]
            contains = [display for display in displays if (query in display or query in display_map.get(display, "")) and display not in starts_with]
    ...
    def _show_top_instruments(self, slot_name: str) -> None:
        ...
        if pair_input is not None and not pair_input.text().strip():
    ...
    def _set_exchange(self, slot_name: str, exchange: str, title: str) -> None:
        if self.coordinator is not None:
            self.coordinator.unsubscribe_public_quote(slot_name)
        self._slot_state[slot_name].update({"exchange": exchange, "market_type": None, "symbol": None})
        ...
        self._apply_fixed_direction_tone()
        self._update_runtime_controls()
        self._emit(f"position_manager:exchange:{slot_name}:{exchange}")
    ...
    def _set_market_type(self, slot_name: str, market_type: str, title: str) -> None:
        ...
        self._update_runtime_controls()
        self._emit(f"position_manager:market_type:{slot_name}:{market_type}")
    ...
    def _set_symbol(self, slot_name: str, symbol: str, title: str) -> None:
        ...
        self._clear_quotes(slot_name)
        self._update_runtime_controls()
        self._emit(f"position_manager:instrument:{slot_name}:{symbol}")
    ...
    def _clear_symbol(self, slot_name: str) -> None:
        ...
        pair_input.setPlaceholderText(tr("spread.enter_symbol") if self._slot_state[slot_name]["market_type"] else tr("spread.choose_instrument"))
        self._pair_completers[slot_name].popup().hide()
        self._update_pair_clear_action(slot_name)
        self._clear_quotes(slot_name)
        self._render_public_quote_spread_view()
        self._update_runtime_controls()
        self._emit(f"position_manager:instrument_cleared:{slot_name}")
        self._update_pair_suggestions(slot_name, "", force_popup=True)
    ...
    def _subscribe_slot(self, slot_name: str) -> None:
        if self.coordinator is None:
            return
        state = self._slot_state[slot_name]
        if not all((state["exchange"], state["market_type"], state["symbol"])):
            return
        self.coordinator.subscribe_public_quote(slot_name, exchange=state["exchange"], market_type=state["market_type"], symbol=state["symbol"])
        self._emit(f"position_manager:subscribe:{slot_name}:{state['exchange']}:{state['market_type']}:{state['symbol']}")

    def _subscribe_both(self) -> None:
        self._subscribe_slot("left")
        self._subscribe_slot("right")
        self._emit("position_manager:select")
    ...
    def _on_public_quote_error(self, message: str) -> None:
        self._emit(f"position_manager:error:{message}")
    ...
    def _render_quote(self, slot_name: str, quote_data: dict) -> None:
        ...
        self._set_label_text_if_changed(labels["bid_price"], self._format_ui_price(bid_price))
        self._set_label_text_if_changed(labels["ask_price"], self._format_ui_price(ask_price))
        ...
        if bid_decimal is not None and ask_decimal is not None:
            self._latest_public_quotes[slot_name] = {"bid": bid_decimal, "ask": ask_decimal}
        self._render_public_quote_spread_view()
    ...
    def _render_public_quote_spread_view(self) -> None:
        ...
        if left_quote is None or right_quote is None:
            ...
            self._apply_fixed_direction_tone()
            return
        self._spread_value_label.setText(self._format_spread_percent(self._safe_edge(right_quote.get("bid"), left_quote.get("ask"))))
        self._apply_fixed_direction_tone()

    def _start_runtime_placeholder(self) -> None:
        self._emit("position_manager:start_placeholder")

    def _stop_runtime_placeholder(self) -> None:
        self._emit("position_manager:stop_placeholder")

    def _update_runtime_controls(self) -> None:
        ready = self._ui_state.is_ready()
        if self._select_button is not None:
            self._select_button.setEnabled(bool(ready))

    def _apply_fixed_direction_tone(self) -> None:
        self._apply_button_tone(self._exchange_buttons.get("left"), "cheap")
        self._apply_button_tone(self._exchange_buttons.get("right"), "expensive")
    ...
    def retranslate_ui(self) -> None:
        for slot_name, button in self._exchange_buttons.items():
            if not self._slot_state[slot_name]["exchange"]:
                button.setText(tr("position_manager.choose_buy_exchange") if slot_name == "left" else tr("position_manager.choose_sell_exchange"))
    ...
    def apply_theme(self) -> None:
        ...
        self.container.setStyleSheet(f"""
            QFrame#spreadContainer {{ background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_alt, 0.96)}, stop: 1 {self._rgba(c_window, 0.98)}); border: 1px solid {self._rgba(c_border, 0.58)}; border-radius: 12px; }}
            QPushButton#exchangeSelector {{ background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_accent, 0.18)}, stop: 0.50 {self._rgba(c_alt, 0.95)}, stop: 1 {c_surface}); color: {c_primary}; border: 1px solid {self._rgba(c_accent, 0.52)}; border-radius: 22px; min-height: 40px; font-size: 12px; font-weight: 700; padding: 6px 12px; text-align: center; }}
            QPushButton#subSelector {{ background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_accent, 0.16)}, stop: 0.50 {self._rgba(c_alt, 0.95)}, stop: 1 {c_surface}); color: {c_primary}; border: 1px solid {self._rgba(c_accent, 0.48)}; border-radius: 18px; min-height: 32px; font-size: 11px; font-weight: 700; padding: 4px 10px; text-align: center; }}
            QPushButton#exchangeSelector:hover, QPushButton#subSelector:hover {{ border: 1px solid {self._rgba(c_accent, 0.70)}; background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_accent, 0.24)}, stop: 0.50 {self._rgba(c_alt, 0.97)}, stop: 1 {self._rgba(c_surface, 0.98)}); }}
            QPushButton#exchangeSelector:pressed, QPushButton#subSelector:pressed {{ border: 1px solid {self._rgba(c_accent, 0.78)}; background: {self._rgba(c_alt, 0.98)}; padding-top: 7px; padding-bottom: 5px; }}
            QPushButton#exchangeSelector[toneRole="cheap"] {{ border: 1px solid {self._rgba(c_success, 0.76)}; background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_success, 0.20)}, stop: 0.50 {self._rgba(c_alt, 0.95)}, stop: 1 {c_surface}); }}
            QPushButton#exchangeSelector[toneRole="expensive"] {{ border: 1px solid {self._rgba(c_danger, 0.76)}; background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_danger, 0.18)}, stop: 0.50 {self._rgba(c_alt, 0.95)}, stop: 1 {c_surface}); }}
            QLineEdit#pairSelector {{ background-color: {c_alt}; color: {c_primary}; border: 1px solid {self._rgba(c_accent, 0.48)}; border-radius: 12px; min-height: 24px; padding: 4px 8px; font-size: 11px; font-weight: 600; }}
            QWidget#transportWidget {{ background: transparent; border: none; }}
            QLabel#transportDot {{ color: {c_muted}; font-size: 15px; font-weight: 900; min-width: 10px; max-width: 10px; background: transparent; border: none; padding: 0; }}
            QLabel#transportDot[activeState="active"] {{ color: #18e06f; }}
            QLabel#transportDot[activeState="inactive"] {{ color: {c_muted}; }}
            QPushButton#transportBadge {{ background-color: transparent; color: {c_muted}; border: 1px solid {self._rgba(c_border, 0.72)}; border-radius: 9px; padding: 0 10px; font-size: 11px; font-weight: 700; min-height: 20px; }}
            QPushButton#transportBadge[activeState="active"] {{ color: {c_primary}; border: 1px solid {self._rgba(c_accent, 0.95)}; }}
            QFrame#quoteSideCapsule {{ background: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 {self._rgba(c_alt, 0.92)}, stop: 1 {self._rgba(c_surface, 0.98)}); border: 1px solid {self._rgba(c_border, 0.58)}; border-radius: 12px; min-height: 34px; }}
            QFrame#quoteMidDivider {{ background-color: {self._rgba(c_border, 0.42)}; border: none; min-height: 16px; max-height: 16px; }}
            QFrame#spreadValueFrame {{ background-color: {self._rgba(c_surface, 0.96)}; border-top: 1px solid {self._rgba(c_border, 0.62)}; border-bottom: 1px solid {self._rgba(c_border, 0.62)}; border-left: 3px solid {self._rgba(c_success, 0.92)}; border-right: 3px solid {self._rgba(c_danger, 0.92)}; border-radius: 14px; }}
            QFrame#spreadValueInner {{ background-color: {self._rgba(c_alt, 0.95)}; border: none; border-radius: 12px; }}
            QPushButton#spreadActionButton {{ background-color: {self._rgba(c_surface, 0.84)}; color: {c_primary}; border: 1px solid {self._rgba(c_border, 0.74)}; border-radius: 14px; font-size: 20px; font-weight: 700; padding: 6px 12px; }}
            QPushButton#spreadActionButton:hover {{ background-color: {self._rgba(c_alt, 0.94)}; border: 1px solid {self._rgba(c_accent, 0.72)}; }}
            QPushButton#spreadActionButton:pressed {{ background-color: {self._rgba(c_alt, 0.98)}; border: 1px solid {self._rgba(c_accent, 0.82)}; padding-top: 7px; padding-bottom: 5px; }}
            QPushButton#spreadModeButton, QPushButton#spreadSimulationButton {{ background-color: {self._rgba(c_alt, 0.94)}; color: {c_primary}; border: 1px solid {self._rgba(c_border, 0.72)}; border-radius: 10px; font-size: 11px; font-weight: 800; padding: 4px 10px; }}
            QLabel#spreadValueLabel {{ color: {c_primary}; background-color: transparent; border: none; font-size: 56px; font-weight: 800; }}
            QLabel#bidLabelText {{ color: {theme_color('success')}; font-size: 11px; font-weight: 700; }}
            QLabel#askLabelText {{ color: {theme_color('danger')}; font-size: 11px; font-weight: 700; }}
            QLabel#quoteQtyLabelText, QLabel#quoteValueText {{ color: {c_primary}; font-size: 11px; font-weight: 700; }}
            QFrame#strategyPanel {{ background-color: {self._rgba(c_surface, 0.72)}; border: 1px solid {self._rgba(c_border, 0.48)}; border-radius: 10px; }}
            QLabel#strategyTitle {{ color: {c_primary}; font-size: 11px; font-weight: 700; padding-left: 4px; }}
            QFrame#strategyFieldCapsule {{ background: qlineargradient(x1: 1, y1: 0, x2: 0, y2: 0, stop: 0 {self._rgba(c_surface, 0.70)}, stop: 1 {self._rgba(c_alt, 0.94)}); border: 1px solid {self._rgba(c_border, 0.60)}; border-radius: 8px; }}
            QLabel#strategyFieldInlineLabel {{ color: {c_muted}; font-size: 10px; font-weight: 600; padding-left: 4px; }}
            QFrame#strategyFieldDivider {{ background-color: {self._rgba(c_border, 0.62)}; border: none; min-height: 14px; max-height: 14px; }}
            QLineEdit#strategyFieldInput {{ background-color: transparent; color: {c_primary}; border: 1px solid {self._rgba(c_border, 0.68)}; border-radius: 9px; min-height: 20px; padding: 0 10px; font-size: 12px; font-weight: 700; }}
            QScrollArea#runtimesScroll {{ background: transparent; border: none; }}
            QScrollArea#runtimesScroll > QWidget > QWidget {{ background: transparent; }}
        """)
        self._apply_fixed_direction_tone()
